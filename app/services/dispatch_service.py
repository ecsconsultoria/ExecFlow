...
def _base_query(company_id: int):
    """Common base query with N+1-safe eager loading.

    ServiceOrder declares assigned_driver/assigned_vehicle/quote as
    lazy="joined". Template uses assigned_driver. supplier is only a column
    (supplier_id), so no relationship to load there.
    """
